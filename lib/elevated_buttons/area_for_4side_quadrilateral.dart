...
class AreaFor4SideQuadrilateral extends StatefulWidget {
  const AreaFor4SideQuadrilateral({Key? key}) : super(key: key);

  @override
  _AreaFor4SideQuadrilateralState createState() =>
      _AreaFor4SideQuadrilateralState();
}

class _AreaFor4SideQuadrilateralState extends State<AreaFor4SideQuadrilateral> {
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      const Text('S(ABCD)='),
                      const Text('V=(xa*yb)+(xb*yc)+(xc*yd)+(xd*ya)'),
                      const Text('V\'=(xb*ya)+(xc*yb)+(xd*yc)+(xa*yd)'),
                      const Text('S=|V-V\'|/2'),
                      Text('=${calculate}')
                    ],
                  ),
    ...
    if (checkData()) {
      ...
      setState(() {
        ...
        ScaffoldMessenger.of(context)
            .showSnackBar(snackBar('Calculated!', Colors.green));
      });
    } else {
      ScaffoldMessenger.of(context)
          .showSnackBar(snackBar('Error,Enter required value!', Colors.red));
    }
  }
  ...
  double equ() {
    ...
    double v1 = (xa * yb) + (xb * yc) + (xc * yd) + (xd * ya);
    double v2 = (xb * ya) + (xc * yb) + (xd * yc) + (xa * yd);
